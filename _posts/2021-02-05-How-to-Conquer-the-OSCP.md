...
I can almost sense the, "Duh, this is so obvious" eye-rolls, but I can tell you that I have attempted Googling a specific service with using one keyword, coming up short and thinking nothing was there, only to come back to it and use different keywords. Even more specific, and why I mentioned this, I was doing a machine on Hack The Box and Googled something like "VulnerableService 2.0.21 exploit". Nothing came up, so I moved on. When I couldn't get anywhere on the machine, I came back to the service and Googled for "VulnerableService 2.0.21 python" and was able to find a python script on Packet Storm.

#### Run Your Recon More Effectively
Probably my favorite trick which helped me save tons of time was by running [Interlace](https://github.com/codingo/Interlace) in conjunction with [nmapAutomator](https://github.com/21y4d/nmapAutomator). Interlace essentially creates single-threaded scripts or other applications and allows for them to be multi-threaded. On the other hand, nmapAutomator is a super underrated reconnaissance script. To highlight some of its features:
...
Get the idea? It's fantastic and a massive time-saver -- especially when time is of the essence. The primitive way would be to run a bash one-liner on all your targets:

```bash
for i in $(cat domains.txt); do bash nmapautomator.sh $i All; done
```

But, why not make it spawn different threads for each line in domains.txt and have them finish much faster? It's very easy to do with Interlace and can be done in the background while you're doing other things. I used something similar to: 
...
By no means does this mean you have to do this. Do what you think will work best for you. However, this is done for several reasons, but most importantly, you can run your recon in the background while you go for the buffer overflow. Not to mention, if you follow the steps in the tips section regarding Buffer Overflow, this should be your easiest machine worth the most points.
